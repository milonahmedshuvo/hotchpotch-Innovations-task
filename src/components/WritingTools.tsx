import GradientButton from "./GradientButton"


const WritingTools = () => {
  return (
    <div>
        <p>title</p>



        <div className="my-20 flex flex-col justify-center lg:items-center gap-y-10 lg:gap-x-40 w-full lg:flex-row">



        <div className="">
    <h1 className="content-title text-primaryColor leading-none">Write what you want</h1>
    <h1 className="content-title text-primaryColor leading-none">convey through clear,</h1>
    <h1 className="content-title text-primaryColor leading-none">authentic writing</h1>


    <p className="text-[16px] md:text-[18px] text-secondaryColor font-inter mt-6 md:mt-8 right-site-peragrap">150k+ users. No Credit Card Required. Pro designs and </p>

    <p className="text-[16px] md:text-[18px] text-secondaryColor font-inter right-site-peragrap">writing at no cost. Start for free. Ai Writer Tool | Generate</p>
    <p className="text-[16px] md:text-[18px] text-secondaryColor font-inter right-site-peragrap">text for ecom, social media, website, sales, blogs etc.</p>

    <div className="mt-6 md:mt-8">
        <GradientButton>tart 14 Days Free Tria</GradientButton>
    </div>
</div>






<div className="border flex gap-4 w-full lg:w-1/3 text-primaryColor p-4 bg-primaryColor rounded-[7px] md:rounded-[12px]">
   

   <div className="w-1/4">
       <ul className="text-[#B7B8BB]">
        <li >Blog Outline</li>
        <li >Blog Intro</li>
        <li >Blog Conclusion</li>
        <li >AIDA</li>
        <li >PAS</li>
        <li >Content Rewriter</li>
       </ul>

       <div className="space-y-3 mt-5">
    <div className="h-1 w-11/12 bg-[#DDE2ED] rounded-[3px]"></div>
    <div className="h-1 w-10/12 bg-[#DDE2ED] rounded-md"></div>
    <div className="h-1 w-9/12 bg-[#DDE2ED] rounded-md"></div>
    </div>

    <div className="space-y-3 mt-2">
    <div className="h-1 w-11/12 bg-[#DDE2ED] rounded-[3px]"></div>
    <div className="h-1 w-10/12 bg-[#DDE2ED] rounded-md"></div>
    <div className="h-1 w-9/12 bg-[#DDE2ED] rounded-md"></div>
    </div>
   </div>


   <div className="w-3/4">
   <h1 className="text-darkColor font-semibold font-inter">Results</h1>
   <p className="text-[#818488] content-peragrap">Here what our AI came up with.</p>

   <div className="bg-darkColor w-full p-4 rounded-md mt-3">
   <div className="h-1 w-1/2 bg-gray-400 rounded-md mb-4"></div>
   <div className="space-y-2">
    <div className="h-1 w-5/6 bg-gray-400 rounded-[3px]"></div>
    <div className="h-1 w-4/6 bg-gray-400 rounded-md"></div>
    <div className="h-1 w-3/6 bg-[#DDE2ED] rounded-md"></div>
    </div>
   </div>


   <div className="h-1 w-1/2 bg-[#DDE2ED] rounded-md mb-3 mt-5"></div>
   <div className="space-y-1">
    <div className="h-1 w-11/12 bg-[#DDE2ED] rounded-[3px]"></div>
    <div className="h-1 w-10/12 bg-[#DDE2ED] rounded-md"></div>
    <div className="h-1 w-9/12 bg-[#DDE2ED] rounded-md"></div>
    </div>

    <div className="h-1 w-1/2 bg-[#DDE2ED] rounded-md mb-3 mt-5"></div>
   <div className="space-y-1">
    <div className="h-1 w-11/12 bg-[#DDE2ED] rounded-[3px]"></div>
    <div className="h-1 w-10/12 bg-[#DDE2ED] rounded-md"></div>
    <div className="h-1 w-9/12 bg-[#DDE2ED] rounded-md"></div>
    </div>

   </div> 
</div>





</div>


    </div>
  )
}

export default WritingTools